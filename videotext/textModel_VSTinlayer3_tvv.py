...
class MultiHeadedAttentionComponent(nn.Module):
    ...
    def __init__(self, hid_dim, n_heads, dropout,dim):
        super().__init__()
        ...
        self.fc_k = nn.Linear(dim*2**2, hid_dim)

        # Transformation from source vector to value vector
        self.fc_v = nn.Linear(dim*2**2, hid_dim)
...
class CrossAttentionLayer(nn.Module):
    ...
    def __init__(self, hid_dim, n_heads, pf_dim, dropout,dim,num_layers,audio_lenth):
        super().__init__()
        self.dim=dim
        ...
        self.ff_layer_norm = nn.LayerNorm(hid_dim)  # Layer norm after FeedForward component
        self.hid_dim = hid_dim
        self.self_attention = MultiHeadedAttentionComponent(hid_dim, n_heads, dropout,dim)
        self.feed_forward = FeedForwardComponent(hid_dim, pf_dim, dropout)
...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_embed3d = PatchEmbed3D(
            patch_size=patch_size, in_chans=videodim, embed_dim=embed_dim,
        ...
        # print(embed_dim*2**(fusionLayer-1))
        # stochastic depth
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule
        self.cross_attention = nn.ModuleList(
            [CrossAttentionLayer(hid_dim, n_heads, pf_dim, audiodrop,embed_dim,fusionLayer,audio_lenth)for _ in range(audio_representation_layers)] )
#         self.cross_attention = nn.ModuleList(
        ...
        for layers in self.layers:
            ...
            if i == 3:
                ...
                for layer in  self.cross_attention:
                    x = layer(video.contiguous(),text.contiguous())
