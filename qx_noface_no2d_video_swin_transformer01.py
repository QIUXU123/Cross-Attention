...
class MultiHeadedAttentionComponent(nn.Module):
    ...
    def __init__(self, hid_dim, n_heads, dropout,dim,fusionLayer):
        super().__init__()
        ...
        self.fc_k = nn.Linear(dim*2**fusionLayer, hid_dim)

        # Transformation from source vector to value vector
        self.fc_v = nn.Linear(dim*2**fusionLayer, hid_dim)
...
class CrossAttentionLayer(nn.Module):
    ...
    def __init__(self, hid_dim, n_heads, pf_dim, dropout,dim,num_layers,audio_lenth,fusionLayer):
        super().__init__()
        self.dim=dim
        ...
        self.ff_layer_norm = nn.LayerNorm(hid_dim)  # Layer norm after FeedForward component
        self.hid_dim = hid_dim
        self.self_attention = MultiHeadedAttentionComponent(hid_dim, n_heads, dropout,dim,fusionLayer)
        self.feed_forward = FeedForwardComponent(hid_dim, pf_dim, dropout)
...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.cross_attention = nn.ModuleList(
            [CrossAttentionLayer(hid_dim, n_heads, pf_dim, audiodrop,embed_dim,self.num_layers,audio_lenth,fusionLayer) for _ in range(audio_representation_layers)])
        ...
        self.custom_layer_1 = nn.Linear(int(embed_dim * 2**self.num_layers), custom_layer_1outdim)
