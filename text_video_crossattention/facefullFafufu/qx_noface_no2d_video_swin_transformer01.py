...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, video, face):
        # text : [batch_size, text_len, hid_dim]
        # audio : [batch_size, audio_len, hid_dim

        # get self-attention
        _video, _ = self.self_attention(face,video , video)
...
#             [CrossAttentionLayer(hid_dim, n_heads, pf_dim, dropout) for _ in range(cross_attention_layers)])
    ...
    def forward(self, video,face):
        """Forward function."""
        face = face.permute(0,4,1,2,3)
        face = self.r2plus1d_18(face)
        face = self.patch_embed3d(face)
        face = self.pos_drop(face)
        for layer in self.facelayers:
            face = layer(face)
        face = rearrange(face,"B C D H W  -> B (D H W) C")
        ...
        for layers in self.layers:
            ...
            if i == 3:
                ...
                for layer in  self.cross_attention:
                    x = layer(x.contiguous(),face.contiguous())
