...
class ConvPosEnc(nn.Module):
    ...
    def forward(self, x, size: Tuple[int, int,int]):
        ...
        feat = x.reshape(B,-1,D,H,W)
        feat = self.proj(feat)
        # print("x",x.shape)
        # # print("feat",feat.shape)
        # print(size)
        # feat = feat.flatten(2).transpose(1, 2)
        x = x.reshape(B,-1,D,H,W) + feat.reshape(B,-1,D,H,W)
        return x
...
class PatchEmbed3D(nn.Module):
    ...
    """
    def __init__(self, patch_size=(4,4,4), in_chans=3, embed_dim=192, norm_layer=None):
        super().__init__()
    ...
    def forward(self, x,size):
        ...
        # padding
        D, Wh, Ww = size
        if Ww % self.patch_size[2] != 0:
            x = F.pad(x, (0, self.patch_size[2] - Ww % self.patch_size[2]))
        if Wh % self.patch_size[1] != 0:
            x = F.pad(x, (0, 0, 0, self.patch_size[1] - Wh % self.patch_size[1]))
...
class ChannelBlock(nn.Module):
    ...
    def forward(self, x, size):
        ...
        cur = self.norm1(x)
        cur = self.attn(cur)
        cur=cur.reshape(B,-1,D,H,W)
        x = x + self.drop_path(cur).permute(0,2,3,4,1)
...
def window_partition(x, window_size):
    ...
    # print("1",x.shape)
    B, C, D, H, W = x.shape
    x = x.reshape(B, math.ceil(D / window_size[0]), window_size[0],math.ceil( H / window_size[1]), window_size[1], math.ceil(W / window_size[2]), window_size[2], C)
...
def window_reverse(windows, window_size, B, D, H, W):
    ...
    Returns:
        x: (B, D, H, W, C)
    """
    x = windows.view(B,math.ceil(D / window_size[0]) , math.ceil(H / window_size[1]), math.ceil(W / window_size[2]), window_size[0], window_size[1], window_size[2], -1)
...
class SpatialBlock(nn.Module):
    ...
    def forward(self, x, size):
        B, C,D,H, W = x.shape
        window_size = get_window_size((D, H, W), self.window_size, None)
        sum1 = B*C*D*H*W
        D,H, W = size
        C = sum1//B//D//H//W
        shortcut = self.cpe[0](x, size)
        shortcut = shortcut.permute(0,2,3,4,1)
        x = self.norm1(shortcut)
        # x = x.permute(0,4,1,2,3)

        pad_l = pad_t = pad_d0 = 0
        pad_t = (self.window_size[0] - D % self.window_size[0]) % self.window_size[0]
        pad_r = (self.window_size[1] - W % self.window_size[1]) % self.window_size[1]
        pad_b = (self.window_size[2] - H % self.window_size[2]) % self.window_size[2]
        # print(x.shape)
        x = F.pad(x, (0, 0, pad_l, pad_r, pad_t, pad_b, pad_d0, pad_t))
        x = x.permute(0,4,1,2,3)
        B,Tp, Hp, Wp,_ = x.shape
        ...
        # x = x.view(B, D*H * W, C)
        shortcut = shortcut.permute(0,4,1,2,3)
...
class DaViT(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, depths=(2, 2, 6, 2), patch_size=(4,4,4),
                 embed_dims=(96, 192, 384, 768), num_heads=(2, 4, 8, 16), window_size=(2,7,7), mlp_ratio=4.,
        ...
        self.patch_embeds = nn.ModuleList([
            PatchEmbed3D(patch_size=patch_size if i == 0 else patch_size,
    ...
    def forward(self, x):
        x = x.permute(0,4,1,2,3)
        size = (x.size(2),x.size(3),x.size(4))
        x, size = self.patch_embeds[0](x,size)
        features = [x]
        sizes = [size]
        branches = [0]
        ...
        for block_index, block_param in enumerate(self.architecture):
            ...
            for branch_id in branch_ids:
                if branch_id not in branches:
                    x, size = self.patch_embeds[branch_id](features[-1].permute(0,4,1,2,3), sizes[-1])
                    features.append(x)
                    sizes.append(size)
                    branches.append(branch_id)
            for layer_index, branch_id in enumerate(block_param):
                features[branch_id], _ = self.main_blocks[block_index][layer_index](features[branch_id].permute(0,4,1,2,3), sizes[branch_id])

        features[-1] = self.avgpool(features[-1])
