...
class MultiHeadedAttentionComponent(nn.Module):
    ...
    def __init__(self, hid_dim, n_heads, dropout,dim):
        ...
        self.fc_k = nn.Linear(dim*2**3, hid_dim)

        # Transformation from source vector to value vector
        self.fc_v = nn.Linear(dim*2**3, hid_dim)
...
#             [CrossAttentionLayer(hid_dim, n_heads, pf_dim, dropout) for _ in range(cross_attention_layers)])
    ...
    def forward(self, video,audio):
        ...
        for layers in self.layers:
            ...
            i+=1
        # video = rearrange(video, 'b c d h w -> b d h w c')
        # # video = self.videodim0(video)
        # # video = self.videodim1(video)
        # for layer in  self.cross_attention:
        #     x =layer(video,audio)
        # x = rearrange(x,'B H W C T -> B H W T C')
        x = self.norm1(x)
